...
// 根据vehicle dynamic and control中的前馈项得到的，
// 这个函数的输入参数是planning trajectory的曲率，而参考的转弯半径就为 R = 1/ref_curvature
double LatController::ComputeFeedForward(double ref_curvature) const {
  const double kv =
      lr_ * mass_ / 2 / cf_ / wheelbase_ - lf_ * mass_ / 2 / cr_ / wheelbase_;
  // understeer gradient：Kv 不足转向梯度
...
void LatController::ComputeLateralErrors(
  ...
  // 全局坐标系下 的向量[dx,dy] 按照目标点的航向角旋转 theta 角 ：
  // [xnew;ynew] = [cos -sin; sin cos] * [x;y]
  ...
  if (VehicleStateProvider::Instance()->gear() ==
    ...
    auto lookahead_point = trajectory_analyzer.QueryNearestPointByRelativeTime(
        ...
        lookahead_station_ /
            ...
            // lookahead_station:
            // 当航向角偏差为0时，车子将会按照目标轨迹运动：
            //    这样前视距离(lookahead_station_) = 当前速度(linear_v) * 前视时间 
            // 当航向角偏差不为0时，车子将以这个偏差偏离目标轨迹运动：
            //    这样前视距离(lookahead_station_) = 当前速度(linear_v)*cos(head_error) * 前视时间 
    heading_error_feedback = common::math::NormalizeAngle(
        ...
        // 考虑lookahead后，反馈的航向角偏差为：当前时刻车辆航向角 与 前视点的航向角 之差
        // 注意：heading_error 还是 当前 与 目标 的航向角之差
  }
  ...
  // 根据几何关系得到的 lateral_error_feedback : 但是为什么是lateral_error(与目标点间的)+lookhead_station*sin(heading_error)
